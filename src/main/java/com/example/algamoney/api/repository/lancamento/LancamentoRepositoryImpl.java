...
public class LancamentoRepositoryImpl implements LancamentoRepositoryQuery {
	...
	public Page<Lancamento> filtrar(LancamentoFilter lancamentoFilter, Pageable pageable) {
		...
		List<Lancamento> lancamentosEncontrados = query.getResultList();
		
		return new PageImpl<>(lancamentosEncontrados, pageable, total(lancamentoFilter));
	}
	...
	@Override
	public List<LancamentoEstatisticaCategoria> porCategoria(LocalDate mesReferencia, TipoLancamento tipoLancamento) {
		CriteriaBuilder criteriaBuilder = manager.getCriteriaBuilder();
		...
		criteriaQuery.where(
				// Sugestão do monitor para retornar apenas categorias relaciondas a despesas
				//Para retornar todos os tipos remover a linha abaixo
			criteriaBuilder.equal(root.get(Lancamento_.tipo), tipoLancamento), 
			criteriaBuilder.greaterThanOrEqualTo(root.get(Lancamento_.dataVencimento),
					primeiroDia),
			criteriaBuilder.lessThanOrEqualTo(root.get(Lancamento_.dataVencimento),
					ultimoDia));
